 ...
 *   GPL Version 2 License http://www.opensource.org/licenses/mit-license.php
...
/* File: jstree.js
    ...
    The only required part of jstree it consists of a few functions bound to the
    $.jstree object, the actual plugin function and a few core functions for
    manipulating a tree.
...
/* Group: $.jstree.
    ...
    Some static functions and variables, unless you know exactly what you are
    doing do not use these, but <$().jstree> instead.
*/
(function ($) {
    var instances            = [],
        focused_instance    = -1,
    ...
    /* 
        Variable: $.jstree
        *object* Contains all static functions and variables used by jstree,
        *some plugins also append variables.
    */
    $.jstree = { 
        /* 
        ...
        */
        ...
        /* 
            Function: $.jstree.__construct
                Creates a new jstree instance, any arguments after the first one
                are merged and used to configure the tree.

                `.data("jstree")` is also called on the container and is used
                for configuration (keep in mind you can specify this data using
                a "data-jstree" attribute)
            Parameters:
                container - *mixed* the container of the tree (this should not
                be the UL node, but a wrapper) - DOM node, jQuery object or
                selector
        */
	    ...
	    // Gather objects specified in the 'arguments' and 'data-jstree'
            // attribute of the container, and use them to initialize the 's'
            // variable (the settings).
            $.extend.apply(null, [true, s].concat(
		Array.prototype.slice.call(arguments, 1),
                (container.data("jstree") || {}) ));

	    // 'slice' with no arguments makes a shallow copy of an array.
            p = $.isArray(s.plugins) ? s.plugins : $.jstree.defaults.plugins.slice();

            p = $.vakata.array_unique(p);
            s = $.extend(true, {}, $.jstree.defaults, s);
            $.each(plugins, function (i, val) { 
		// Shouldn't this be: 'if val !== core' ? XXX
                if(i !== "core" && $.inArray(i, p) === -1) { s[i] = null; delete s[i]; } 
                else { t.push(i); d[i] = {}; }
            });
            s.plugins = t;

	    // Set 'i' to the index of the newly created empty instance.
	    ...
	    //
	    // This function is invoked with the "Constructor Invocation Pattern".  
	    // I.e. "new $.jstree.__construct(this, settings)".  Thus a new object
	    // is created and 'self' is set to that object.  Notice how we then
	    // set instances[i] = 'this'.  Tricky.
	    //
            this.data = d;
		...
		return container.children("ul:eq(0)");
            ...
            this.get_settings = function (writable) { 
		return writable ? s : $.extend(true, {}, s);
	    };
            ...
            instances[i] = this;
            $.each(t, function(j, val) {
		if (plugins[val]) {
		    plugins[val].__construct.apply(instances[i]);
		}
	    });
        ...
        /*
            ...
            Function: $.jstree.__destruct
                Destroys an instance, and also clears `jstree-` prefixed classes
                and all events in the `jstree` namespace

            Parameters:
                instance - *mixed* the instance to destroy (this argument is
                passed to <$.jstree._reference> to get the instance)
        ...
        */
		...
		operation - *string* the operation to execute args - *array* the
        ...
        */
        ...
        /* 
            ...
            Parameters:
                needle - *mixed* - integer, DOM node contained inside a jstree
                container, ID string, jQuery object, selector
        */
        ...
        /*
            Function: $.jstree._focused
                ...
                Returns the currently focused instance (by default once an
                instance is created it is focused)
        ...
        */
        ...
        /*
            Function: $.jstree._focus
                Make an instance focused (which defocuses the previously focused
                instance)

            Parameters:
                instance - *mixed* the instance to focus (this argument is
                passed to <$.jstree._reference> to get the instance)
        ...
        */
        ...
        /*
            ...
            Parameters:
                ...
                plugin_name - *string* the name of the new plugin (it will be
                    used as a key in an object - make sure it is valid)
                plugin_data - *object* consists of 4 keys. Default is:
                    ...
                    >    _fn            : { },    // each key of this object should be a function
                                                  // that will extend the jstree prototype
        ...
        */
        plugin: function (plugin_name, plugin_data) {
            ...
            $.each(plugin_data._fn, function (i, val) {
		        ...
		        // When this function is called, 'this' should be an
		        // instance (created by new $.jstree.__construct).
                    ...
                    if (i.indexOf("_") === 0) {
                        ...
                        rslt = this.__trigger(evnt,
			    { "func" : i, "args" : args, "plugin" : func.plugin });
				        ...
				        (replace_arguments ? Array.prototype.slice.call(arguments, 1) : args ) );
                                }
                            }), args);
                    }
                    return rslt;
        ...
        /*
            Variable: $.jstree.defaults
                ...
                *object* storing all the default configuration options for every
                plugin and the core.  If this is modified all instances created
                after the modification, which do not explicitly specify some
                other value will use the new default.
    ...
    /* Group: $().jstree()
        ...
        The actual plugin wrapper, use this to create instances or execute
        functions on created instances.

        Function: $().jstree

        Creates an instance using the specified objects for containers, or
        executes a command on an instance, specified by a container.

        Parameters:
            settings - *mixed* 
            
            - if you pass an *object* a new instance will be created (using
            <$.jstree.__construct>) for each of the objects in the jquery
            collection, if an instance already exists on the container it will
            be destroyed first
            
            - if you pass a *string* it will be executed using <$.jstree.__call>
              on each instance
...
(function ($) {
    ...
    $.jstree.plugin("core", {
        ...
        */
        ...
        _fn : { 
            ...
            */
            ...
            /* 
                ...
                This function corrects the open/closed/leaf state as data
                changes (as the user interacts with the tree).  The core calls
                this function automatically when a node is opened, deleted or
                moved.

                Parameters:

                    obj - *mixed* this is used as a jquery selector - can be
                    jQuery object, DOM node, string, etc. If _-1_ is used or is
                    omitted the root nodes are processed.

                Returns:

                    jQuery collection - the processed children of the original
                    node.
            */

            /* PROCESS SINGLE NODE (OR USE BOOLEAN single PARAM), CALL FROM
	     * CLEAN_NODE, LOSE THE EVENTS ABOVE */
                ...
                obj.each(function () {
                    ...
                    switch(!0) {
                        case obj.hasClass("jstree-open") && !obj.find("> ul > li").length:
                            obj.removeClass("jstree-open").addClass("jstree-leaf")
			      .children("ul").remove(); // children("ins").html("&#160;").end()
                            break;
	    ...
	    send_node: function(obj) {
		alert("Send");
	    },
	    share_node: function(obj) {
		alert("Share");
            }
        }
    });
